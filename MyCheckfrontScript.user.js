...
(function($) {
    ...
    const PAGEUTILS = (function(){
        ...
        return {
            get_$contentDiv:getContentDiv,
            get_$mainPageContentAfterHeaders:getMainPageContentAfterHeaders,
            get_$pageTitle:findPageTitle,
            get_$primaryActionsDiv:findPrimaryActionsDiv,
            get_$secondaryActionsDiv:findSecondaryActionsDiv,
        }
    })();
...
`;
    ...
    const DAILYMANIFEST = (function(){
        ...
        function replaceDailyManifestWith($element, title){
            const $pageTitle = PAGEUTILS.get_$pageTitle()
            if(needRevert === false){
            ...
            if(needRevert === false){
                ...
                const $actions = PAGEUTILS.get_$primaryActionsDiv();
            ...
            const $page = PAGEUTILS.get_$mainPageContentAfterHeaders();
            if(needRevert === false){
...
//--------------------------------------------------------------------------
        ...
        function MakeCheckinsReport(){
            ...
            function MakeTable(checkingInAr){
                ...
                replaceDailyManifestWith($table,'Daily Checkins');
                        ...
                        for(var j = 1; j < salesReport.length; j++){
                            ...
                            if((B.bookingCode == S['Booking Code']) &&
                               ...
                               ((B.startDateYMD == S['Item Start Date']) ||
                                ...
                                $em = $(`<em>${DAILYMANIFEST.export_utils.roomType(S)}</em>`);
    ...
    const GUEST_FORMS = (function(){
        function DoMakeSimpleGuestList(){
            ...
            function GenerateSimpleGuestList(...args){
                ...
                PROGRESS.setMessage(`Generating table...`);
        ...
        return {
            generateSimpleTable:DoMakeSimpleGuestList,
            downloadImportTemplate:DoGenerateGuestImportTemplate,
            importCsv:DoGuestImportCSV,
        }
    })();
...
`,
    ...
    const CHANGEOVER_HEADERS = [['Room', '{Product Name}'],
                                ['Next checkin',(i,j,k,hv,row)=>DAILYMANIFEST.export_utils.daysTillNext(row)],
                                ['Single / Twin / Double', (i,j,k,hv,row)=>DAILYMANIFEST.export_utils.roomTypeNext(row)],
                                ['Accessability needs',(i,j,k,hv,row)=>DAILYMANIFEST.export_utils.accessabilityReqNext(row)]
                               ];
...
`);})(); //End GM_addStyle//
    ...
    //= These are the functions that modify the pages and provide the extra features
    //========================================================================================
    ...
    function AddDailyManifestReportButtons(){
        ...
        const $a = cloneActionButtons($newBookingButton_desktop, $newBookingButton_mobile, 'overnightButton', 'Overnight Report', ICONS.MOON, DAILYMANIFEST.reports.overnight);
        const $b = cloneActionButtons($newBookingButton_desktop, $newBookingButton_mobile, 'changeOverButton', 'Changeover Report', ICONS.BED, DAILYMANIFEST.reports.changeOver);
        const $c = cloneActionButtons($newBookingButton_desktop, $newBookingButton_mobile, 'dailyCheckinsButton', 'Days Checkins', ICONS.CHECKIN, DAILYMANIFEST.reports.checkins);
    ...
    function AddScrollHelpersToGuestPages(){
        ...
        if($flashDiv.length === 0){
            $flashDiv = $(`<div id="flashing-arrow-thing" title="Scroll to guest data">${ICONS.ARROW.UP}</div>`);
    ...
    function AddSimpleGuestlistButton(){
        ...
        $sidebar.append($btn);
        $btn.on('click',function(event){event.preventDefault(); GUEST_FORMS.generateSimpleTable();});
    }
    ...
    function AddGuestImportButton(){
        ...
        (()=>{
            ...
            $sidebar.append($btn);
            $btn.on('click',function(event){event.preventDefault(); GUEST_FORMS.downloadImportTemplate();});
        })();
    ...
    //========== These functions should perform the steps to actually mode the page ==========

    function DoMods_DailyManifest(){
    ...
    //*******************************
    //* Map page locations to functions to call to mod that page
    //* The key may be a regular expression (or at least implement the function "test")
    //* or be a function.
    //*******************************
    const modmap = new Map(
        [
            [/booking\/manifest/,DoMods_DailyManifest],
            [/calendar\/customer/,DoMods_CustomerCalendar],
            [/booking\/[A-Z]{4}-\d{6}\/guests/,DoMods_GuestForms],
    ]);
    ...
    function DoMods(){
        ...
        console.log(L);
        for (const [key, value] of modmap.entries()) {
            if(typeof key === 'fucntion'){
                if(key(L)){
                    value.call(this);
                }
            }else if(key.test(L)){
